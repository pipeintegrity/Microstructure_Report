...
cvn_rec <-
  recipe(CVN_US_FS ~ iitys_5+ c + mn + s + si + p + ags + pdp + mli,
         data = complete) %>%
  step_center(all_predictors(),-mli) %>%
  # step_scale(all_predictors()) %>%
  step_interact(terms = ~ c(mn:c, ags:pdp, c:si)) %>%
  prep() #retain data =TRUE is default
...
mli_lm <- lm(CVN_US_FS ~ sqrt(mli), data = complete)

mli_lm2 <- update(mli_lm, . ~ . + mn)# no good
mli_lm3 <- update(mli_lm, . ~ . + mn:c) # significant
mli_lm4 <- update(mli_lm, . ~ . + pdp/ags + c) # no good
mli_lm5 <- update(mli_lm, . ~ . + pdp/ags + c:s)
...
mli_aug <- augment(mli_lm4, newdata=complete)
mli_aug2 <- augment(mli_lm5, newdata=complete)
...
sulfur <- mli_aug2 %>% 
  ...
  scale_color_brewer(type = "div",palette = "Set1")+
   labs(title = "Observed CVN vs. Predicted",
       ...
       caption = "Model: CVN ~ (MLI)^0.5 + DP + AGS:DP + C:S",
       color="Method")
...
carbon <- mli_aug %>% 
  ...
  scale_color_brewer(type = "div",palette = "Set1")+
   labs(title = "Observed CVN vs. Predicted",
       x="Predicted CVN (ft-lb)",
       y = "Full-Size US CVN (ft-lb)",
       caption = "Model: CVN ~ (MLI)^0.5 + DP + AGS:DP + C",
       color="Method")
